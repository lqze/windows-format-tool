...
class MainApp(Frame):
    ...
    def start_format(self, drive, firmware_folder):
        ...
        try:
            ...
            self.logbox.insert(INSERT, 'Successfuly copied {} to \
                                location {}\\'.format(firmware_folder, drive))
        except Exception as e:
            self.logbox.insert(INSERT, "Error copying file. Reason:\n" + str(e))
            self.logbox.insert(INSERT, "Consider manually copying the file.")
    ...
    def btn_start_callback(self):
        ...
        if not firmware_str:
            tkinter.messagebox.showerror("No Folder Selected" ,
                            "Select a firmware location first.")
        else:
            currentDrive = self.drive_menu.get()[0:2]
            b = tkinter.messagebox.askyesno("Format Disk?" ,
    ...
    def select_drive_menu(self, frame):
        ...
        selection = OptionMenu(frame, var1, *drivestrs)
        selection.pack()
        selection.configure(width=32)
        selection.place(relx=0.1, rely=0.11, anchor=NW)
